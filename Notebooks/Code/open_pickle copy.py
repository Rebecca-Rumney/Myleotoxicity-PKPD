...
with open("Images/PK_sim/PK_comp1/large_data_" + str(data_set) + "_2pop/pop_V_c_ll_profiles_quad_approx_data.pkl", "rb") as fp:
...
for graph_id, data in pickle_data.items():
    method, param = graph_id
    print(method, param)
    if method == 'no_quad_approx.':
        data['Result'] = np.asarray(data['Result'])
        res = data['Result']
...
time_table_data = np.zeros((3, 3))
error_table_data = np.zeros((3, 3))
last_param = None
prev_method = None
method_arg = -1
method_names = []
colours = pxcol.qualitative.Safe
for method, param in pickle_data.keys():
    data = pickle_data[(method, param)]
    time = data['Time']
    time_min = time/60
...
fig.write_image("Images/PK_sim/PK_comp1/large_data_" + str(data_set) + "_2pop/pop_V_c_ll_profiles_quad_approx_error_graph.svg")
fig.show()
...
fig.write_image("Images/PK_sim/PK_comp1/large_data_" + str(data_set) + "_2pop/pop_V_c_ll_profiles_quad_approx_time_table.svg")
fig.show()
...
fig.write_image("Images/PK_sim/PK_comp1/large_data_" + str(data_set) + "_2pop/pop_V_c_ll_profiles_quad_approx_error_table.svg")
fig.show()
